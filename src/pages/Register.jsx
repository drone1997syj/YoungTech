...
export default function Register() {
  const { register, verifyRegisterEmail, loginWithGoogle, navigate, user } = useShop();
  ...
  const handleVerifyEmail = async (e) => {
    ...
    if (!verification?.verificationId || verificationCode.replace(/\D/g, '').length !== 6) {
      setError('이메일 인증번호 6자리를 입력해 주세요.');
      return;
    }

    setSubmitting(true);
    const res = await verifyRegisterEmail(verification.verificationId, verificationCode);
    setSubmitting(false);

    if (res.success) {
      setSuccess(true);
      setVerification(null);
      alert('이메일 인증이 완료되어 회원가입이 완료되었습니다. 로그인해 주세요.');
      navigate('login');
      return;
    }

    setError(res.message || '이메일 인증에 실패했습니다.');
  };
      ...
      <div className="auth-card card p-8 max-w-md w-full">
        ...
        <div className="auth-header text-center mb-8">
          <div className="auth-icon mx-auto mb-3">
            {verification ? <MailCheck size={24} /> : <UserPlus size={24} />}
          </div>
          <h2 className="font-extrabold text-dark text-xl mb-1">
            {verification ? '이메일 인증' : '영테크 회원가입'}
          </h2>
          <p className="text-xs text-light">
            {verification
              ? '가입 이메일의 소유자 확인이 필요합니다.'
              : '안전한 주문 관리를 위해 새 계정을 생성합니다.'}
          </p>
        </div>

        {error && <div className="alert-box alert-danger mb-4 text-xs font-semibold">{error}</div>}
        {success && <div className="alert-box alert-success mb-4 text-xs font-semibold">회원가입이 완료되었습니다.</div>}

        {verification ? (
          <form onSubmit={handleVerifyEmail} className="auth-form flex flex-col gap-4">
            <div className="signup-verification-card">
              <div className="signup-verification-icon">
                <ShieldCheck size={20} />
              </div>
              <div>
                <b>{verification.maskedEmail || email}</b>
                <span>위 이메일로 보낸 6자리 인증번호를 입력해 주세요.</span>
              </div>
            </div>

            <div className="form-group">
              <label className="form-label text-xs font-bold text-dark mb-1">이메일 인증번호 *</label>
              <input
                type="text"
                className="form-input text-sm text-center tracking-widest"
                ...
                required
              />
              <p className="form-help-text">인증번호는 10분 동안 유효하며, 5회 실패 시 다시 가입 인증을 요청해야 합니다.</p>
            </div>

            {verification.devVerificationCode && (
              <div className="signup-dev-code">
                개발 테스트용 인증번호: <b>{verification.devVerificationCode}</b>
              </div>
            )}
            ...
            <div className="flex gap-2 mt-2">
              <button
                type="button"
                className="btn btn-secondary flex-1 py-2.5 font-bold text-sm"
                onClick={() => {
                  setVerification(null);
                  setVerificationCode('');
                  setError('');
          ...
          <>
            ...
            <div className="social-auth-container mb-4">
              <button onClick={handleNaverLogin} className="btn btn-naver" disabled={submitting}>
              ...
              <button onClick={handleKakaoLogin} className="btn btn-kakao" disabled={submitting}>
              ...
              <button onClick={handleGoogleLogin} className="btn btn-google" disabled={submitting}>
