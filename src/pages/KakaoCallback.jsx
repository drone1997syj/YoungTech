...
export default function KakaoCallback() {
  const { loginWithKakao, navigate } = useShop();
  const [status, setStatus] = useState('카카오 로그인 요청을 확인하고 있습니다...');
  ...
  useEffect(() => {
    const handleCallback = async () => {
      ...
      try {
        setStatus('카카오 인증 정보를 바탕으로 계정을 연동하고 있습니다...');
        const res = await loginWithKakao(code, state);
        ...
        if (res.success) {
          if (res.linked) {
            alert('💡 계정 연동 안내\n\n기존 가입하신 이메일 계정이 확인되어, 해당 카카오 소셜 계정과 안전하게 자동 연동되었습니다.');
          }
  ...
  return (
    <div className="container py-24 flex flex-col items-center justify-center text-center animate-fade-in">
      <div className="card p-12 max-w-sm w-full flex flex-col items-center gap-6">
        ...
        <h3 className="font-bold text-dark text-base">카카오 간편 로그인 처리 중</h3>
      ...
      <SocialAccountLinkModal
        ...
        onLinked={() => {
          alert('카카오 간편로그인이 기존 영테크 계정에 연결되었습니다.');
          navigate('home');
        }}
      />
    </div>
